...
img = Image.new("RGB", (SIZE, SIZE), color=(0, 0, 0))  # черный фон, без прозрачности
...
with TelegramClient(StringSession(session), api_id, api_hash) as client:
    ...
    # Загружаем и применяем как профильное фото
    result = client(functions.photos.UploadProfilePhotoRequest(file=file))
